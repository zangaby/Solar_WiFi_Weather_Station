...
float measured_UVI;
String UVI_risk;                     // Risk according to the UVI
float measured_temp;
float measured_humi;
float measured_pres;
float adjusted_temp;
...
void setup() {
  ...
  if (App1 == "BLYNK") {
    ...
    Blynk.begin(auth, ssid, pass, bserver, port);
  }
  if (App2 == "THINGSPEAK") {
...
//**********************************VEML6075****************************************
  ...
  if (App1 == "BLYNK") {
    ...
    Blynk.virtualWrite(11, UVI_risk);                // virtual pin 11
  ...
  if (App2 == "THINGSPEAK") {
    ...
    ThingSpeak.setField(8, measured_UVI);
...
void measurementEvent() {
  ...
  // Get UVI
  measured_UVI = uv.readUVI();
  Serial.print("UV Index reading: "); Serial.println(measured_UVI);
 
 
  char strCat[12];
  byte categ = (byte)(measured_UVI+0.5);  //round up
  if ((categ >= 0) && (categ < 3)) {
    strcpy(strCat, "LOW"); 
    ...
    strcpy(strCat, "HIGH !");
  } else if ((categ >= 8) && (categ < 11)) {
     strcpy(strCat, "VERY HIGH!");
  } else if (categ >= 11) {
    strcpy(strCat, "EXTREME!");
  }

  UVI_risk = String(measured_UVI) +" "+strCat; 
  Serial.println(UVI_risk);
